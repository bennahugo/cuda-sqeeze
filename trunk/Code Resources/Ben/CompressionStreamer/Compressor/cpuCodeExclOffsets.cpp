...
void compressionKernel(const float * data, uint32_t elementCount, uint32_t dataBlockIndex, 
    ...
    uint32_t sizeOfPrefixArray = (elementsInDataBlock/2 * bitCountForRepresentation) / storageIndiceCapacity +
                                 ((elementsInDataBlock/2 * bitCountForRepresentation) % storageIndiceCapacity != 0);
	...
	if (index % 2 != 0) {
            ...
            uint32_t iTimesBitCountForRepresentation = (i/2)*bitCountForRepresentation;
...
void decompressionKernel(uint32_t chunkSize, uint32_t dataBlockSize, 
	...
	uint32_t index = lowerBound+i;
        if (index % 2 != 0) {
            //inflate prefix
            uint32_t prefixIndex = i/2*bitCountForRepresentation;
	    ...
	    uint32_t count = 32;
            ...
            residual |=
                ( compressedResiduals[startingIndex+(storageIndiceCapacity - residuallshiftAmount - writtenBits > 0)] >> (residuallshiftAmount + writtenBits - 1) >> 1);
            _decompressorIV[index] ^= residual;
            accumulatedIndex += count;
        }
    }
}
