...
216~x~4096~x~112~x~2 32-bit floats (756.0 MiB in total) is shown in figure~\ref{ALG_THROUGPUT}. Similar results are 
...
in figure~\ref{ALG_UTILITY_COMPARISON}. The experiment was run using the same 756.0 MiB file used for the throughput experiment. 
The utilities was using default compression and decompression settings. Only around 0.39s was spent on compression and 0.18s on decompression. The rest of the time 
is spent reading from the original HDF5 files, writing a compressed file, reading the compressed file and finally writing a decompressed file.
\begin{figure}[ht!]
 \begin{mdframed}
 \centering
 \begin{subfigure}[b]{\textwidth}
