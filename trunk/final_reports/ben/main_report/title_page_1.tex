...
END SUBROUTINE 
\end{verbatim}
The decompression algorithm is remarkably similar except that the leading zero count does is not computed, but instead retrieved from the prefix array. The shift
operations are inverted to retrieve the bits that may be split over two consecutive elements in both the prefix and residual arrays. The prediction step is moved towards
the end of the routine, where the predicted value is XORed with the residual to restore the original, uncompressed, value.
 ...
 Considerable savings in compression ratio will, however, only be achieved for extrapolation of \textit{smooth data}.
 
 If a function $f:[1\dots n]\rightarrow\mathbb{R}$ is evaluated over a time interval $[1\dots n]$ and the results are stored in a sequence $a_1\dots a_n$, where 
 $a_i = f(i)$
  \begin{figure}[h!]
    \centering
...
FUNCTION INT32 pivotMedian(INT32[] data, int n) {
 ...
 \subsubsection{Lagrange prediction}
  Lagrange extrapolation prediction is a gradiant-based method that can be used to predict a value of a polynomial at $t+1$, given the values at $t\dots t-n$.
  The method is commonly used for interpolation between known points on a curve defined by some polynomial. Good extrapolations is only possible for 
  \textit{smooth} data.
  ...
  accuracy from extrapolating from the previous \textit{m} values. Such smooth sequences normally arrise as solutions of ordanary differential equations (ODEs) in
  ...
  the method works just as well if these differences vary in length, as long as such sequence can be approximated some polynomial \cite{engelson2000lossless}. The method 
  builds from \textit{Lagrange's Rule} for an extrapopolation. The rule states, that for a given function, $f$ implicitly defined over the previous \textit{m} 
  ...
  can be computed by the following equation below. Note that $x^{*} = m + 1$, the index of the unknown value of f, after the $1\dots m$ previously observed values.
  \begin{equation}
