...
perform addition over the remainder the elements of A 
  ...
  Due to the fact that both the packing and unpacking algorithm is so similar in their construction for the sake of time constraints only the packing algorithm was implemented in CUDA. A 
  general compacting kernel requires the use of atomics as mentioned earlier, but still achieves up to 6.1 GiB/s on a GT670.
