...
\begin{mdframed}
 ...
 \begin{tikzpicture}(1,1)
  ...
  \draw [fill=green,green] (2.55,0.57) rectangle (4.8,4.47);
  \draw [fill=orange,orange] (3.95,3.4) rectangle (5.35,5.6);
...
Previous research suggests several different constructions of predictors. The use of a \textit{Lorenzo} predictor \cite{lindstrom2006fast,CGF:CGF681} generalizes the well known parallelogram predictor by
...
The implementations by \cite{O'Neil:2011:FDC:1964179.1964189,1607248,4589203,4976448,engelson2000lossless} target \textit{double} precision floating-point data which is described by the IEEE 754 standard (revised 2008).
...
IEEE 754 \textit{single} precision floating-point values (this is due to the fact that the correlation step in the MeerKAT pipeline transforms the data into 32-bit values). 
...
An exclusive or operation is a \textit{binary} operation (taking two operands). It returns false if both operands are the same. If $\oplus$ represents 
...
Our algorithm is very similar, but operates on correlated 32-bit data and stores the difference between corresponding values at time $t-1$ and \textit{t}. It is almost \textit{symmetrical} in its 
...
The algorithm structure in figure~\ref{PACKING_ALGORITHM} is used because it allows for easy swapping between different predictors. The predictor in this case 
...
approaches taken in previous research, specifically that of Klein, Burtscher \& O'Neil et al. \cite{4976448,O'Neil:2011:FDC:1964179.1964189,klein2005parallel}
 \subsubsection{Element-wise parallel execution}
 ...
 This scan operation can be computed in parallel as suggested by Ladner, Cole and Blelloch et al. \cite{ladner1980parallel,cole1989faster,blelloch1990prefix}.
...
Programming for GPU architectures brings about a host of associated challenges. For starters, each generation of GPUs have widely differing architectures, each making many architecture specific optimizations. Each generation, for instance, 
...
It should be noted that the headers cannot be compressed with the algorithm being proposed and an LZ variant / entropy encoder may be better tailored for this task. 
...
\end{figure}
 ...
 \subsection{Algorithm parallelization and its implications for GPGPU architectures}
  ...
  The alternative is to convert the basic algorithm into an element-wise parallel algorithm. As mentioned in the design chapter this requires computing a prefix sum  
  ...
  The up-sweep operation divides the array into pairs of elements and assigns each pair to a processor. It computes partial sums for each pair and 
...
//and n is strictly greater than 0
 ...
 This distribution step can be avoided by splitting the dataset into blocks, where each block is a multiple of the warp size in length. This effectively means that the data can be
 ...
 One way to avoid two atomic operations per residual is to use an array of bytes as the atomic unit of the residual stream. However, since each element is 4 bytes in length, this may involve anything from 0 up to 4 global memory
 ...
 \subsection{Feasibility analysis}
  ...
  The Lagrange predictor is accurate for smooth data that can be well approximated by some polynomial of degree \textit{n}, for example a sinusoid. However, the noise present in the KAT-7 data renders this
  ...
  The next major problems are the limited number of 128-bit registers available, as well as the fact that the packing algorithm does not have the same number of outputs as inputs. Although
  ...
  Currently, the number of correlated antennae is limited to 7+1 sources, each with 2 polarizations. However, as the number of antennae grow (to around 3000) the number of correlations will exhibit 
  ...
  Unfortunately, none of these approaches made a significant improvement in terms of throughput, although pinned memory showed a small 200-300 MiB/s increase in throughput if a large number of observations are transfered between the 
...
Performance Cluster at the University of Cape Town. These nodes have four 16-core processors clocked at 2200 MHz with 128 GiB primary memory, clocked at 1333 MHz. The average algorithm throughput excluding any disk I/O, for a file of 
216~x~4096~x~112~x~2 32-bit floats (756.0 MiB in total, excluding header information) is shown in figure~\ref{ALG_THROUGPUT}. The average compression ratio achieved is 91.02\% with a sample standard deviation of 2.01\%.
\begin{figure}[ht!]
 \begin{mdframed}
 \centering
 \begin{tikzpicture}
  ...
  \end{tikzpicture}
  \caption[Throughput results on a 64 core AMD Opteron]{Algorithm performance using up to four 16-core AMD Opteron 6274 CPUs}
  \label{ALG_THROUGPUT}
  \end{mdframed}
\end{figure}
...
\begin{figure}[ht!]
 ...
 \begin{tikzpicture}
  ...
  \caption[Comparison to standard utilities]{Comparison between the predictive scheme and the standard utilities GZIP, BZIP2 and ZIP}
...
\begin{figure}[ht!]
 ...
 \begin{tikzpicture}
  \pgfplotstableread{ % Read the data into a table macro
	...
	1		0.0072		0.1587168	0.3275123047
	4		0.02449		0.6040148	1.0272662109
	8		0.046784	1.080432	2.9061765625
	16		0.073802	1.855014	5.192562793
  }\datatable
  ...
  \begin{axis}[
    ...
    \addplot [every node near coord/.append style={font=\tiny,shift={(20pt,-3pt)}},mark=square*, color=red] table [x=Threads, y=Huffman] {\datatable};
    \addplot [every node near coord/.append style={font=\tiny,shift={(-10pt,-3pt)}},mark=triangle*, color=blue] table [x=Threads, y=Predictive] {\datatable};
 ...
 \begin{tikzpicture}
  \pgfplotstableread{ % Read the data into a table macro
	Threads		Huffman		Predictive	ZLE
	1		0.020975	0.3731228	0.4102287109
	4		0.027808	1.36565		1.5583790039
	8		0.029639	2.436506	3.1913351563
	16		0.03158743	4.415304	5.4654459961
  }\datatable
  ...
  \caption[Comparison between predictive compressor, parallel Huffman and ZLE compressors]{Comparison between Predictive Compressor, ZLE and Huffman coding (excluding Disk I/O)}
  \label{CONCURRENT_WORK}
  \end{mdframed}
\end{figure}
\subsection{Algorithm development and improvements}
...
\begin{figure}[ht!]
 ...
 \begin{tikzpicture}
  \pgfplotstableread{ % Read the data into a table macro
	Approach				Compression	Decompression
	{OMP element-parallel}			0.400		0.600		
	{Posix Blocked}				0.673		1.178
	{OMP Blocked}				1.98		3.66			
	{LZCNT Bit-twiddled}			2.97		5.15
	{GT780 (kernel)} 			13.6		n/a
	{GT780 (kernel + Sync Pin MemCpy)} 	0.70		n/a
  }\datatable
  ...
  \begin{axis}[
    ...
    ymin=0,         % Start y axis at 0
    xtick=data,     % Use as many tick labels as x coordinates
    xticklabels from table={\datatable}{Approach},  % Get the labels from the Threads column of the \datatable
    x label style={shift={(0pt,-65pt)}},
  ...
  \end{axis}  
  \end{tikzpicture}
  \caption{Comparison between several iterations of the implementation using 16 cores. The LZCNT optimizations are run on a slower AMD Opteron processor, but still outperforms a faster
...
\begin{enumerate}
 ...
 this goal was not attainable, but that the predictive compressor only produced compression ratios of 9\% on average, in contrast to compression ratios of 39\%.
...
After investigating improvements in throughput focus shifted to improving the average compression ratio achieved, by taking the difference between successive samples. Such a basic difference
