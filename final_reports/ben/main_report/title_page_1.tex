...
FUNCTION INT32 pivotMedian(INT32[] data, int n) {
 ...
 \subsection{Algorithm parallelization}
  ...
  \begin{verbatim}
   ...
   END SUBROUTINE
  \end{verbatim}
  Blelloch suggests a novel approach \cite{blelloch1990prefix} to compute this prefix scan in parallel. The algorithm treats the input array of integers as a \textit{complete}
  binary tree, meaning that each level of the binary tree is completely filled, except for the last level of the tree. The algorithm requires the size of the input array to
  be a power of 2. If this is not the case padding has to be added to the input array. The algorithm is done in 3 primary steps, with a synchronization point between each. First
  an \textit{up-sweep} operation is completed, then the last element is set to 0, and finally a \textit{down-sweep} operation is completed on the tree. Each level of the tree
  can be done in parallel and therefore the algorithm has a complexity of $O(n/p + \log_2{p})$ where p is the number of processors in a system. 
